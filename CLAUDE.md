...
- `arohcp-ci-triage` — CI fleet triage tool. 4 commands: `survey` (fleet health — signatures, envelopes, cross-env correlation, baseline pass rates, failure stage split, ARM error codes; `--format=compact` for LLM consumption at 13x token reduction, `--format=summary` for 170x reduction fleet dashboard), `triage RUN` (single-run deep extraction — per-test failures, step timings, all infra signals, neighbor context), `dig RUN WHAT` (per-test artifact drill-down: `tests`, `azure`, `metrics`, `podinfo`, `events`, `provision`, `alerts`, `pool`, `steptime`, `links`), `watch` (fleet monitor with per-PR advisories — classifies each PR as infra/code/mixed/latest_passed; `--pr=N` for single-PR check in <2s). Powers `/cidig` and `/ciscan` skills. Zero external deps. Build: `go build -o /tmp/arohcp-ci-triage ./tooling/arohcp-ci-triage/`. All output is JSON.
