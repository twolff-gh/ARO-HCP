...
func BuildHCPClusterFromParamsV20251223(
	...
	cluster := hcpsdk20251223preview.HcpOpenShiftCluster{
		...
		Properties: &hcpsdk20251223preview.HcpOpenShiftClusterProperties{
			...
			DNS: &hcpsdk20251223preview.DNSProfile{
				BaseDomainPrefix: to.Ptr(parameters.ClusterName),
			},
			Platform: &hcpsdk20251223preview.PlatformProfile{
				ManagedResourceGroup:   to.Ptr(parameters.ManagedResourceGroupName),
				NetworkSecurityGroupID: to.Ptr(parameters.NsgResourceID),
				SubnetID:               to.Ptr(parameters.SubnetResourceID),
				OutboundType:           to.Ptr(hcpsdk20251223preview.OutboundTypeLoadBalancer),
				OperatorsAuthentication: &hcpsdk20251223preview.OperatorsAuthenticationProfile{
					UserAssignedIdentities: parameters.UserAssignedIdentitiesProfile,
				},
			},
	...
	// Set KeyVault visibility if specified (for private KeyVault support)
	if parameters.KeyVaultVisibility != "" {
		cluster.Properties.Etcd.DataEncryption.CustomerManaged.Kms.Visibility = to.Ptr(hcpsdk20251223preview.KeyVaultVisibility(parameters.KeyVaultVisibility))
	}

	// Set Autoscaling if specified
	if parameters.Autoscaling != nil {
		cluster.Properties.Autoscaling = parameters.Autoscaling
	}

	return cluster
}

// CreateHCPClusterAndWaitV20251223 creates a v20251223preview HCP cluster and waits for completion
func CreateHCPClusterAndWaitV20251223(
	ctx context.Context,
	logger logr.Logger,
	hcpClient *hcpsdk20251223preview.HcpOpenShiftClustersClient,
	resourceGroupName string,
	...
	ctx, cancel := context.WithTimeoutCause(ctx, timeout, fmt.Errorf("timeout '%f' minutes exceeded during CreateHCPClusterAndWaitV20251223 for cluster %s in resource group %s", timeout.Minutes(), clusterName, resourceGroupName))
	defer cancel()

	logger.Info("Creating cluster", "cluster", clusterName, "resourceGroup", resourceGroupName)
	poller, err := hcpClient.BeginCreateOrUpdate(ctx, resourceGroupName, clusterName, cluster, nil)
	if err != nil {
		return nil, fmt.Errorf("failed to initiate cluster creation for %q in resourcegroup=%q: %w", clusterName, resourceGroupName, err)
	}

	logger.Info("Waiting for cluster creation to complete", "cluster", clusterName)
	operationResult, err := poller.PollUntilDone(ctx, &runtime.PollUntilDoneOptions{
		Frequency: StandardPollInterval,
	})
	if err != nil {
		return nil, fmt.Errorf("failed waiting for cluster %q in resourcegroup=%q to finish creating: %w", clusterName, resourceGroupName, err)
	}

	switch m := any(operationResult).(type) {
	case hcpsdk20251223preview.HcpOpenShiftClustersClientCreateOrUpdateResponse:
		expectResp, err := GetHCPClusterV20251223(ctx, hcpClient, resourceGroupName, clusterName)
		if err != nil {
			if errors.Is(err, context.DeadlineExceeded) {
				return nil, fmt.Errorf("failed to get cluster, caused by: %w, error: %w", context.Cause(ctx), err)
			}
			return nil, err
		}
		err = checkOperationResult(&expectResp.HcpOpenShiftCluster, &m.HcpOpenShiftCluster)
		if err != nil {
			return nil, err
		}
		return &m.HcpOpenShiftCluster, nil
...
// GetHCPClusterV20251223 gets a v20251223preview HCP cluster
func GetHCPClusterV20251223(
	ctx context.Context,
	hcpClient *hcpsdk20251223preview.HcpOpenShiftClustersClient,
	resourceGroupName string,
	clusterName string,
) (hcpsdk20251223preview.HcpOpenShiftClustersClientGetResponse, error) {
	return hcpClient.Get(ctx, resourceGroupName, clusterName, nil)
}
